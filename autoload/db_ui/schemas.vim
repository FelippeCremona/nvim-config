...
" DB2 (autoload/db/adapter/db2.vim, definido nessa config, não é adaptador
" nativo do vim-dadbod). Usa SYSIBM.SYSTABLES (catálogo do DB2 for z/OS:
" CREATOR = schema, NAME = tabela, TYPE = 'T' pra tabelas de verdade).
"
" O adaptador db2 roda via sqlline com --outputformat=csv, que gera linhas
" tipo 'valor1','valor2' (cada campo entre aspas simples); por isso o
" parser próprio abaixo, em vez do s:results_parser genérico.
function! s:db2_parse_results(results, min_len) abort
  " Só mantém linhas no formato CSV com aspas do sqlline, descarta avisos
  " do jline e linhas em branco.
  let rows = filter(copy(a:results), {_, v -> v =~# "^'.*'$"})
  if len(rows) > 0
    " primeira linha é o cabeçalho (--showHeader=true)
    let rows = rows[1:]
  endif
  ...
  for row in rows
    ...
    let fields = map(fields, {_, f -> trim(substitute(f, "^'\\|'$", '', 'g'))})
    call add(parsed, fields)
  endfor

  if a:min_len ==? 1
    return map(parsed, {_, v -> v[0]})
  endif
  return filter(parsed, {_, v -> len(v) ==? a:min_len})
endfunction
...
" Add ClickHouse and DB2 to the schemas dictionary
let s:schemas = {
